...
import { generateUniqueFileName, UPLOAD_ERROR_MESSAGES, validateFileSize, validateFileType } from '@/config/upload'
 ...
 * @param options 验证选项
 */
function validateFile(file: File, options: R2UploadOptions & { file: File }): void {
  // 验证文件是否存在
  if (!file) {
 ...
 * @returns 上传结果
 */
export async function uploadToR2(options: R2UploadOptions & { file: File }): Promise<R2UploadResult> {
